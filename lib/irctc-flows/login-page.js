...
class LoginPage {
  ...
  async loginWithCaptcha(username, password) {
    try {
      console.log('[LoginPage] Starting login with captcha...');
      ...
      for (const selector of usernameSelectors) {
        try {
          ...
          if (await usernameField.isExisting()) {
            console.log('[LoginPage] Found username field');
            break;
          }
        } catch (e) {}
      }
      ...
      for (const selector of passwordSelectors) {
        try {
          ...
          if (await passwordField.isExisting()) {
            console.log('[LoginPage] Found password field');
            break;
          }
        } catch (e) {}
      }
  ...
  async waitForCaptchaInput() {
    try {
      ...
      console.log('[LoginPage] Waiting 10 seconds for captcha input...');
        ...
        try {
          ...
          if (await captchaField.isExisting()) {
            ...
            if (captchaText && captchaText.trim().length > 0) {
              console.log('[LoginPage] Captcha detected, waiting 2 seconds before login');
    ...
    try {
      ...
      try {
        ...
        if (await stillOnLogin.isExisting()) {
          console.log('[LoginPage] Still on login screen - login may have failed');
          return { success: false, error: 'Login failed - still on login screen. Check captcha or credentials.' };
        }
      } catch (e) {}
      
      console.log('[LoginPage] Login appears successful - moved away from login screen');
