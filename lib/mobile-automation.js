...
class MobileAutomation {
  ...
  async startAppiumServer() {
    return new Promise((resolve, reject) => {
      ...
      const args = ['appium', '--port', '4723', '--address', '127.0.0.1', '--allow-cors'];
      ...
      setTimeout(() => {
        console.warn('[Mobile] Appium start timeout - resolving anyway');
        resolve(child);
      }, 15000);
    });
  }
  ...
  async connect() {
    try {
      ...
      console.log('[Mobile] Activating app...');
      await this.driver.activateApp('cris.org.in.prs.ima');
      await this.driver.pause(2000); // 2s wait from java code
